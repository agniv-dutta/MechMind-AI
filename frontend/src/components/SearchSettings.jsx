...
export default function SearchSettings({ onBackToDashboard: _onBackToDashboard, onNavigateSettings, darkMode }) {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Search Settings</h1>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          {!loading && (
            <>
              ...
              <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-teal-500/10 rounded-2xl p-6 shadow-2xs space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-teal-500/20 rounded-xl text-sm font-semibold text-slate-900 dark:text-slate-100 appearance-none focus:outline-none cursor-not-allowed opacity-70"
                      disabled
                    >
                ...
                <div className="space-y-4 pt-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="bg-teal-100 dark:bg-teal-950 text-teal-800 dark:text-teal-300 text-xs font-mono font-bold px-2.5 py-1 rounded-md border border-teal-500/20">Alpha = {semanticWeight.toFixed(2)}</span>
                  </div>

                  <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200/80 dark:border-teal-500/20">
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                        <span>Semantic (Dense)</span>
                        <span className="font-mono text-teal-600 dark:text-teal-400">{Math.round(semanticWeight * 100)}%</span>
                      </div>
                      <input
                        type="range"
                        min="0.0"
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                        ...
                        <span className="font-mono text-indigo-600 dark:text-indigo-400">{Math.round((1 - semanticWeight) * 100)}%</span>
                      </div>
                      <input
                        type="range"
                        min="0.0"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-mono font-bold text-xs border border-emerald-500/20">{minConfidence}%</span>
                    </div>
                    <input
                      type="range"
                      min="0"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-mono font-bold text-xs text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-teal-500/20">{maxResults}</span>
                    </div>
                    <input
                      type="range"
                      min="1"
              ...
              <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-teal-500/10 rounded-2xl p-6 shadow-2xs space-y-6">
                ...
                <div className="bg-slate-50/70 dark:bg-slate-800/40 border border-slate-200/80 dark:border-teal-500/20 rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <button
                        onClick={() => setQueryExpansion(!queryExpansion)}
                        className={`w-11 h-6 rounded-full transition-colors p-1 ${queryExpansion ? 'bg-[#18224B] dark:bg-teal-500' : 'bg-slate-300 dark:bg-slate-700'}`}
                      >
                        <div className={`w-4 h-4 rounded-full bg-white transition-transform ${queryExpansion ? 'translate-x-5' : 'translate-x-0'}`}></div>
                      </button>
                      <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-sm">Query Expansion</h3>
                    </div>
                  </div>
          ...
          {/* Bottom Action Bar */}
          <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-teal-500/10">
            <div className="text-xs space-y-1 max-w-sm">
              {saveMsg && <span className="flex items-center gap-1.5 font-semibold text-emerald-600 dark:text-emerald-400"><CheckCircle2 className="w-3.5 h-3.5" />{saveMsg}</span>}
              {saveError && <span className="flex items-center gap-1.5 font-semibold text-red-600 dark:text-red-400"><AlertTriangle className="w-3.5 h-3.5" />{saveError}</span>}
            </div>
            <div className="flex items-center gap-3">
              <button onClick={handleReset} className="text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
                Reset to Defaults
              </button>
              <button
                onClick={handleSave}
        ...
        <div className="w-full md:w-[300px] space-y-6 shrink-0">
          <div className="bg-white dark:bg-[#0f172a] border-t-4 border-emerald-600 border-x border-b border-slate-200 dark:border-teal-500/10 rounded-b-2xl rounded-t-lg p-5 space-y-4 shadow-sm">
            <div className="flex items-center space-x-2 font-bold text-slate-900 dark:text-slate-100 text-base">
              <Info className="w-5 h-5 text-emerald-500" />
              <span>Current Configuration</span>
            </div>
            <div className="space-y-2.5 text-xs">
              <div className="flex justify-between items-center pb-1.5 border-b border-slate-100 dark:border-teal-500/10">
                <span className="text-slate-500 dark:text-slate-400">Semantic Weight</span>
                <span className="font-mono text-slate-900 dark:text-slate-100 font-bold">{semanticWeight.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center pb-1.5 border-b border-slate-100 dark:border-teal-500/10">
                <span className="text-slate-500 dark:text-slate-400">Min Confidence</span>
                <span className="font-mono text-slate-900 dark:text-slate-100 font-bold">{minConfidence}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 dark:text-slate-400">Max Results</span>
                <span className="font-mono text-slate-900 dark:text-slate-100 font-bold">{maxResults}</span>
              </div>
            </div>
            <div className="bg-teal-50 dark:bg-slate-800/80 border border-teal-200 dark:border-teal-500/20 rounded-xl p-3 text-xs text-teal-900 dark:text-teal-200 flex items-start gap-2.5 mt-3">
