        ...
        <div className="flex flex-col md:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-11 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/40"
            />
          ...
          <div className="flex items-center space-x-2 w-full md:w-auto">
            <button
              ...
              className="bg-teal-100 text-teal-800 text-xs font-semibold px-4 py-3 rounded-xl flex items-center gap-1.5 hover:bg-teal-200 transition-colors border border-teal-500/20"
            >
            ...
            <button
              onClick={() => runSearch()}
              className="bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold px-6 py-3 rounded-xl shadow-sm transition-colors flex-1 md:flex-initial"
            >
              Search
            </button>
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select value={equipmentClass} onChange={(e) => setEquipmentClass(e.target.value)} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-800 appearance-none focus:outline-none">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select value={docType} onChange={(e) => setDocType(e.target.value)} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-800 appearance-none focus:outline-none">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-4 border-t border-slate-100 gap-4">
              ...
              <div className="flex items-center space-x-2">
                <button onClick={clearAll} className="bg-slate-100 text-slate-700 text-xs font-semibold px-4 py-2 rounded-lg hover:bg-slate-200 transition-colors">Clear All</button>
      ...
      {results && !loading && !error && (
        <div className="space-y-4">
          ...
          {filteredResults.map((r, i) => {
            ...
            return (
              <div key={r.chunk_id || i} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-slate-300 shadow-2xs transition-all space-y-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-9 h-9 rounded-xl bg-blue-500/15 text-blue-600 flex items-center justify-center shrink-0">
                      <FileText className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-900 text-base">{r.source_doc}</h3>
                      <div className="flex items-center space-x-2 text-xs text-slate-400 font-mono mt-0.5">
                        <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 font-bold text-[10px]">PAGE {r.page ?? 1}</span>
                        <span>•</span>
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md bg-slate-100 text-slate-700 text-xs font-semibold">
