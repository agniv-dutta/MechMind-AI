    ...
    >
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          {/* Card 1: Core Engine */}
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-5 relative">
            <span className="absolute top-6 right-6 text-xs font-mono text-slate-400 bg-slate-100 px-2.5 py-1 rounded border border-slate-200/60">
              CFG-01-A
            </span>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 appearance-none focus:outline-none"
                  >
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono text-slate-500 focus:outline-none"
                />
                <button
                  type="button"
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
              ...
              <button
                onClick={handleTest}
                disabled={testing}
                className="bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold px-4 py-2 rounded-xl flex items-center justify-center gap-1.5 transition-colors disabled:opacity-60"
              >
            ...
            {testResult && !testResult.success && (
              <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs font-semibold text-amber-700 flex items-center space-x-2">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2 py-0.5 rounded bg-slate-100 font-mono font-bold text-xs text-slate-900 border border-slate-200">{temperature.toFixed(2)}</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2 py-0.5 rounded bg-slate-100 font-mono font-bold text-xs text-slate-900 border border-slate-200">{maxTokens}</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 font-mono font-bold text-xs border border-emerald-500/20">{citationThreshold}%</span>
              </div>
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-200">
            <div className="text-xs space-y-1">
              {saveMsg && <span className="font-semibold text-emerald-600">{saveMsg}</span>}
              {saveError && <span className="font-semibold text-red-600">{saveError}</span>}
            </div>
            <button
              onClick={handleSave}
              disabled={saving || !model}
              className="bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs px-6 py-3 rounded-xl flex items-center gap-2 shadow-sm transition-colors disabled:opacity-60"
            >
        ...
        <div className="w-full md:w-[300px] space-y-6 shrink-0">
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4 shadow-2xs">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs font-extrabold font-mono ${mode === 'groq' ? 'text-emerald-600' : 'text-amber-600'}`}>{mode.toUpperCase()}</span>
            </div>
            <div className="space-y-2 text-xs">
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-teal-400"></span>
                  <span className="text-slate-600">Groq Inference</span>
                </div>
                <span className={`font-mono font-bold ${mode === 'groq' ? 'text-slate-800' : 'text-slate-400'}`}>{mode === 'groq' ? 'ON' : 'OFF'}</span>
              </div>
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-amber-400"></span>
                  <span className="text-slate-600">Embedded Fallback</span>
                </div>
                <span className="font-mono font-bold text-slate-800">{mode === 'groq' ? 'STANDBY' : 'ACTIVE'}</span>
              </div>
            </div>
          </div>

          <div className="bg-slate-50 border-l-4 border-slate-900 border-y border-r border-slate-200 rounded-r-2xl p-5 space-y-3 shadow-2xs">
            <span className="text-xs font-bold text-slate-700 tracking-wider uppercase block">SYSTEM NOTICE</span>
            ...
            <a href="#" onClick={(e) => e.preventDefault()} className="text-xs font-bold text-slate-900 hover:underline flex items-center gap-1 pt-1">
              <span>Read Documentation</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </a>
          </div>

          <div className="rounded-2xl overflow-hidden relative h-32 border border-slate-200 bg-slate-950 p-3 flex flex-col justify-between shadow-md">
